...
My Bills is local-first. Bills, income, spending, settings, and related finance data are stored in the app's WebView storage on the device. The app does not require the Android INTERNET permission in 20.1.1. User backups are created only through the explicit backup/export flow.
